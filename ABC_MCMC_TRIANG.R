...
abc_mcmc_triangular = function(h, yobs, n) {
  ...
  triangular_kernel = function(h, y, yobs) {
    if (abs(y-yobs) <= h) {return ((h-abs(y-yobs))/h)}
    else {return (0)}
  }
  ...
  while (triangular_kernel(h, initial_y, yobs) == 0) { # if initial y not within bandwidth of h , then re-sample a new p and a new y
  ...
  for (i in 1:N) {
    ...
    acceptanceprob = (triangular_kernel(h, proposed_y, yobs) * dunif(proposed_p, 0, 1) * old_given_new) / 
      (triangular_kernel(h, current_y, yobs) * dunif(current_p, 0, 1) * new_given_old)
...
abc_mcmc_h_1_triang = abc_mcmc_triangular(1, yobs, n)

abc_mcmc_h_2_triang = abc_mcmc_triangular(1, yobs, n)

abc_mcmc_h_3_triang = abc_mcmc_triangular(3, yobs, n)

abc_mcmc_h_1_summary_t = list(mean=mean(unlist(abc_mcmc_h_1_triang)), std = sqrt(var(unlist(abc_mcmc_h_1_triang))))

abc_mcmc_h_2_summary_t = list(mean=mean(unlist(abc_mcmc_h_2_triang)), std = sqrt(var(unlist(abc_mcmc_h_2_triang))))

abc_mcmc_h_3_summary_t = list(mean=mean(unlist(abc_mcmc_h_3_triang)), std = sqrt(var(unlist(abc_mcmc_h_3_triang))))

par(mfrow=c(1,3))

sequence_samples = seq(0, 1, length=1000)

hist(unlist(abc_mcmc_h_1_triang), freq=FALSE, xlab="Samples", main="ABC MCMC")
lines(sequence_samples, dbeta(sequence_samples, posterior_alpha, posterior_beta), type ='l')

hist(unlist(abc_mcmc_h_2_triang), freq=FALSE, xlab="Samples", main="ABC MCMC")
lines(sequence_samples, dbeta(sequence_samples, posterior_alpha, posterior_beta), type ='l')

hist(unlist(abc_mcmc_h_3_triang), freq=FALSE, xlab="Samples", main="ABC MCMC")
lines(sequence_samples, dbeta(sequence_samples, posterior_alpha, posterior_beta), type ='l')
